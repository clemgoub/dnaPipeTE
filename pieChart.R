...
perc=signif(    c(      ((counts$reads[1:(length(counts$reads)-1)])/(counts$reads[length(counts$reads)]) )    , Single_or_low_copy/(counts$reads[length(counts$reads)]*100),    3)      )
...
pie(c(subcounts$reads[1:(length(subcounts$reads)-1)], Single_or_low_copy), paste(perc, "%"), col=c(as.character(subpalette$col),"#d3d3d3ff"), clockwise=T, border="white")
...
#pie(c(counts$reads[1:(length(counts$reads)-1),Single_or_low_copy),paste(perc, "%"), clockwise=T, col=pal, border="white")
#legend(1,0.5,paste(c(as.character(counts$class[-length(counts$reads)]),"Single or low copy DNA")),fill=pal)
