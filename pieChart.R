...
pie(c(counts$reads[1:8],Single_or_low_copy),paste(perc, "%"), col=pal, clockwise=T, border="white")
...
pie(c(counts$reads[1:8],Single_or_low_copy),paste(perc, "%"), clockwise=T, col=pal, border="white")
legend(1,0.5,paste(c(as.character(counts$class[-9]),"Single or low copy DNA")),fill=pal)
