...
echo '##################################################'
echo '### CD-HIT to find Satellites / Simple Repeats ###'
echo '##################################################'
echo''
date +"%T"
...
### Récupère les reads qui n'ont pas eu de hit aux blasts, ni sur les annot, ni sur les NA###
echo 'Selecting unblasted reads for CD-HIT run...'

cat $2/blast_out/sorted.reads_vs_unannoted.blast.out | awk '{print $1}' > $2/CD_HIT/reads_on_NAcontig.header
cat $2/blast_out/sorted.reads_vs_annoted.blast.out | awk '{print $1}' > $2/CD_HIT/reads_on_annot_contig.header
cat $2/CD_HIT/reads_on_* > $2/CD_HIT/all_matching_reads.header

perl -ne 'if(/^>(\S+)/){$c=!$i{$1}}$c?print:chomp;$i{$_}=1 if @ARGV' $2/CD_HIT/all_matching_reads.header $2/renamed.blasting_reads.fasta > $2/CD_HIT/unblasted_reads.fa
...
###CD HIT : 90% d'id sur au moins 50% d'overlapp entre 2 reads###
$CD_HIT -i $2/CD_HIT/unblasted_reads.fa -o $2/CD_HIT/unmatching_clustering -c 0.9 -G 0 -aS 0.5 -aL 0.5 -M 0 -T $3
