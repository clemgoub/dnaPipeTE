   ...
   if (inherits(Colv, "dendrogram")) {
    ...
    if (missing(lmat) || is.null(lmat)) {
        ...
        if (!missing(ColSideColors)) {
            ...
            lhei=c(lhei[1], side.height.fraction*nrow(ColSideColors), lhei[2])
        }
        ...
        if (!missing(RowSideColors)) {
			...
			lwid <- c(lwid[1], side.height.fraction*ncol(RowSideColors), lwid[2])
        }
        lmat[is.na(lmat)] <- 0
    }
    ...
    if (!missing(RowSideColors)) {
        if (!is.matrix(RowSideColors)){
            ...
            if (length(colnames(RowSideColors)) > 0) {  
            	...
            	axis(1, 1:ncol(RowSideColors), labels=colnames(RowSideColors), las=2, cex.axis=cexCol, tick=F, xlab="", ylab="")
