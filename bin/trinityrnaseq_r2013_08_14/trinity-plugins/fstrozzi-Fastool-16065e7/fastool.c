	...
	if (rev_comp) {
		while (kseq_read(seq) >= 0) {
			...
			for(int i = 0; i < seq->seq.l; ++i) {
                          char nextChar = *(seq->seq.s + seq->seq.l-1 - i);
                          if (nextChar == 'A' || nextChar == 'a') rev_seq[i] = 'T';
                          else if (nextChar == 'C' || nextChar == 'c') rev_seq[i] = 'G';
                          else if (nextChar == 'T' || nextChar == 't' ||
                                   nextChar == 'U' || nextChar == 'u') rev_seq[i] = 'A';
                          else if (nextChar == 'G' || nextChar == 'g') rev_seq[i] = 'C';
                          else if (nextChar == 'N' || nextChar == 'n') rev_seq[i] = 'N';
			}
			rev_seq[seq->seq.l] = '\0';
