...
int main(int argc,char** argv)
{
    ...
    int start_file_fd=open(start_file.c_str(),O_CREAT,0666);
    close(start_file_fd);
    ...
    */
    ...
    if (NO_CLEANUP) {
        ...
        if( 0 != rename(start_file.c_str(),finished_file.c_str()) ) {
          cout << "could not rename " << start_file.c_str() << " to " << finished_file.c_str() << endl;
        }
