...
read.table("factors_and_colors")->fac_col

#order factors and colors
land$fam1=factor(land$fam1, levels=as.character(fac_col$V1))

#plot the landscape graph
ggplot(land, aes(div, fill=fam1))+geom_histogram(binwidth=1.1)+labs(list(x="div", y="reads count"))+scale_x_continuous(limits = c(0, 35))+scale_fill_manual(values=as.character(fac_col$V3))
