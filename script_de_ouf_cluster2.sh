...
#$1 = run1 input file (for blasts)
#$2 = outfile
#$3 = cpu
#$4 = /path_to/RM_library.fasta
#$5 = input concatenated file (reads in comp run1 + sample2)
...
echo ''
echo 'ITERATIVE MODE: ITERATION 2'
...
$Trinity --seqType fa --JM 10G --single $5 --CPU $3 --min_glue 0 --output $2
...
$Blast_folder/blastn -query $2/blast_out/unmatching_reads1.fasta -db $2/blast_out/blast2_db.fasta -task dc-megablast -out $2/blast_out/reads_vs_unannoted.blast.out -outfmt 6 -perc_identity 80 -num_threads $3
...
cat $2/single.fa.read_count >> $2/Counts2.txt
