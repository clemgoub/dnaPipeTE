...
if not os.path.isfile('config.ini'):
    ...
    config['DEFAULT'] = {'Trinity': '/opt/dnaPipeTE/bin/trinityrnaseq-Trinity-v2.5.1/Trinity',
                        'Trinity_memory': '',
                        'RepeatMasker': '',
                        'RepeatMasker_library': '',
                        'TRF': '',
                        'Blast_folder': '',
                        'Parallel': '/opt/dnaPipeTE/bin/parallel',
                        'Sample_size': 500000,
                        'RM_species' : "All",
                        'Sample_number': 2,
                        'Trinity_glue' : 1}
...
print( "                      v.1.4c \"container\" 06.Oct.2022                           ")
print( "                                                                                 ")
print( "                                                                                                                                                                       ")
print( "                                                               ")
print( "                                                 :@@@@'     ")
...
class Trinity:
    ...
    def trinity_iteration(self, iteration):
        ...
        trinity = self.Trinity_path+" --seqType fa --max_memory "+str(self.Trinity_memory)+" --single "+self.output_folder+"/"+self.sample_files[iteration]+" --CPU "+str(self.cpu)+" --min_glue "+str(self.trin_glue)+" --min_contig_length "+str(self.contig_length)+" --output "+self.output_folder+"/Trinity_run"+str(iteration+1)
...
class Blast:
    ...
    def blast1_run(self):
        ...
        if not os.path.isfile(self.output_folder+"/Reads_to_components_Rtable.txt") or not os.path.isfile(self.output_folder+"/blast_out/sorted.reads_vs_Trinity.fasta.blast.out") or (os.path.isfile(self.output_folder+"/Reads_to_components_Rtable.txt") and not os.path.getsize(self.output_folder+"/Reads_to_components_Rtable.txt") > 0) or (os.path.isfile(self.output_folder+"/blast_out/sorted.reads_vs_Trinity.fasta.blast.out") and not os.path.getsize(self.output_folder+"/blast_out/sorted.reads_vs_Trinity.fasta.blast.out") > 0):
            ...
            blast += "cat "+self.output_folder+"/blast_out/sorted.reads_vs_Trinity.fasta.blast.out | awk '{print $2\"\\t\"$3\"\\t\"$4}' | sed 's/_c/\tc/g;s/_g/\tg/g' > "+self.output_folder+"/Reads_to_components_Rtable.txt"
            print("blast1 command:")
            print(blast)
            blastProcess = subprocess.Popen(str(blast), shell=True)
            blastProcess.wait()
    ...
    def blast2_run(self):
        ...
        if not os.path.isfile(self.output_folder+"/blast_out/unmatching_reads1.fasta") or (os.path.isfile(self.output_folder+"/blast_out/unmatching_reads1.fasta") and not os.path.getsize(self.output_folder+"/blast_out/unmatching_reads1.fasta") > 0):
            if not os.path.exists(self.output_folder+"/blast_out"):
                os.makedirs(self.output_folder+"/blast_out")
            print("blasting...")
            blast = "cat "+self.output_folder+"/Annotation/annoted.fasta > "+self.output_folder+"/blast_out/blast2_db.fasta && "  #merge annotated contigs and repbase if speciedslib has been generated by a default run of RepeatMasker
...
class Graph:
    ...
    def run(self):
        ...
        graph = "Rscript "+os.path.dirname(os.path.realpath(sys.argv[0]))+"/graph.R "+self.output_folder+" Reads_to_components_Rtable.txt blast_reads.counts "+str(countbase)+" && "
        graph += "awk 'NR > 1' "+self.output_folder+"/reads_per_component_sorted.txt | sort -k1,1 > "+self.output_folder+"/sorted_reads_per_component && "
        graph += "sort -k1,1 "+self.output_folder+"/Annotation/one_RM_hit_per_Trinity_contigs > "+self.output_folder+"/Annotation/one_RM_hit_per_Trinity_contigs_S && "
        graph += "join -a1 -12 -21 "+self.output_folder+"/sorted_reads_per_component "+self.output_folder+"/Annotation/one_RM_hit_per_Trinity_contigs_S -o 1.3,1.5,1.1,2.2,2.4,2.5,2.3 | sort -k1,1nr > "+self.output_folder+"/reads_per_component_and_annotation && "
        #graph += "rm "+self.output_folder+"/reads_per_component_sorted.txt "+self.output_folder+"/sorted_reads_per_component && "
        graph += "Rscript "+os.path.dirname(os.path.realpath(sys.argv[0]))+"/pieChart.R "+self.output_folder+" Counts.txt "+os.path.dirname(os.path.realpath(sys.argv[0]))+"/pieColors && "
