    ...
    def blast2_run(self):
        ...
        if not os.path.isfile(self.output_folder+"/blast_out/unmatching_reads1.fasta") or (os.path.isfile(self.output_folder+"/blast_out/unmatching_reads1.fasta") and not os.path.getsize(self.output_folder+"/blast_out/unmatching_reads1.fasta") > 0):
            if not os.path.exists(self.output_folder+"/blast_out"):
                os.makedirs(self.output_folder+"/blast_out")
            print("blasting...")
            blast = "cat "+self.output_folder+"/Annotation/annoted.fasta "+os.path.dirname(os.path.realpath(sys.argv[0]))+"/bin/RepeatMasker/Libraries/RepeatMasker.lib > "+self.output_folder+"/blast_out/blast2_db.fasta && "  #merge annotated contigs and repbase if speciedslib has been generated by a default run of RepeatMasker
