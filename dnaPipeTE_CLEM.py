...
#parser.add_argument('-lib', action='store', defaut=['DEFAULT']['RepeatMasker_library'], dest='RM_library',)
...
class Graph:
	...
	def run(self):
		...
		graph += "join -12 -21 "+self.output_folder+"/Annotation/sorted_blast3 "+self.output_folder+"/Annotation/one_RM_hit_per_Trinity_contigs -o 1.3,2.4,2.5 | awk '/LINE/ { print $0 \"\\t\" $3; next} /LTR/ {print $0 \"\\t\" $3; next} /SINE/ {print $0 \"\\tSINE\"; next} /DNA/ {print $0 \"\\tDNA\"; next} {print $0 \"\\tOther\"}' | grep 'LINE\|SINE\|LTR\|DNA' | sed 's/Unknow\//DNA\//g' > "+self.output_folder+"/reads_landscape && "
